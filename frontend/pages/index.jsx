  ...
  return (
    <div className="stars-bar">
      {[...Array(max)].map((_, i) => (
        <span 
          key={i} 
          className={`star-slot ${i < count ? 'earned pulse' : ''}`}
        >
          {i < count ? '⭐' : '☆'}
        </span>
      ))}
...
export default function Home() {
  ...
  const handleNext = () => {
    if (currentIndex < SIGNS.length - 1) {
      ...
      // Pour la démo, on donne une étoile à chaque avancement
